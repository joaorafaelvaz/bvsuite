// Validação: Cadência Individual
// Universo: Base P 24m
// Cadência habitual: histórico COMPLETO (para ter mais intervalos)
// 1a Vez: visitas_hist = 1 (sem cadência calculável)
// Exige >=2 visitas históricas para ter ratio
...
const sql = `
  SELECT
    ...
    COUNT(DISTINCT CASE WHEN ci.ratio IS NULL AND ci.total_visitas_hist > 1 THEN ci.cliente END) as sem_ratio
  FROM (
    SELECT
      bs.cliente,
...
console.log('\n=== Cadencia Individual (universo 24m + cadencia hist completa) — Joinville ===');
...
console.log(`Sem ratio (>1 visita mas sem intervalo): ${r.sem_ratio}`);
console.log('');
...
console.log(`1a Vez (hist=1):     ${r.primeira_vez}  (${Math.round(r.primeira_vez/r.total*100)}%)`);
