// Validação: Cadência Individual com cadência habitual calculada DENTRO da janela de 24m
// Universo: Base P 24m com >=3 visitas na janela de 24m
...
// Cadência habitual calculada APENAS dentro da janela de 24m
const sql = `
  SELECT
    ...
    COUNT(DISTINCT CASE WHEN ci.total_visitas_24m = 1 THEN ci.cliente END) as primeira_vez,
  ...
  FROM (
    SELECT
      bs.cliente,
      COALESCE(vh24.total_visitas, 0) as total_visitas_24m,
      uvc.ultima_venda,
...
console.log('\n=== Cadencia Individual (cadencia habitual dentro de 24m) — Joinville ===');
console.log(`Total universo: ${r.total}`);
console.log(`Avg cadencia: ${parseFloat(r.avg_cadencia||0).toFixed(1)}d, Avg dias sem vir: ${parseFloat(r.avg_dias||0).toFixed(1)}d`);
...
console.log(`1a Vez:              ${r.primeira_vez}  (${Math.round(r.primeira_vez/r.total*100)}%)`);
